...
timeof(y,x)=(t=t+x)/1000000''' % (nkeys, program)
                if 'empty' in program:
                    print >>plot, '''
plot '< cat results/%s | grep ^alloc' using 2:3:(log10($3/10)**2/5.0) with lp pt 7 ps variable lw 0.05''' % (program)
                else:
                    print >>plot, '''
plot '< paste results/%s results/hashfct-empty | grep ^alloc' using 2:($3/$6):(log10($3/$6)**2/5.0) with lp pt 7 ps variable lw 0.05''' % (program)
