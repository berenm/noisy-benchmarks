...
plot '< cat results/%s | grep ^alloc' using 2:3:(log10($3/10)**2/5.0) with lp pt 7 ps variable lw 0.05, \\
     '< cat results/%s | grep ^dealloc' using 2:3:(log10($3/10)**2/5.0) with lp pt 7 ps variable lw 0.05''' % (program, program)
                    ...
                    print >>plot, '''
plot '< paste results/%s results/buffer-empty | grep ^alloc' using 2:($3/$6):(log10($3/$6)**2/5.0) with lp pt 7 ps variable lw 0.05, \\
     '< paste results/%s results/buffer-empty | grep ^dealloc' using 2:($3/$6):(log10($3/$6)**2/5.0) with lp pt 7 ps variable lw 0.05''' % (program, program)
